...
async def robot_control():
	...
	while not rospy.is_shutdown():
		...
		send_over_data_channel("Gripper command: {}".format(gripper_command))	
		send_over_data_channel("Gripper value: {}".format(gripper_value))	
		...
		if gripper_command:
			if g == 0:
				...
				while gripper_command and gripper_value > 0:
					...
					send_over_data_channel("[WARNING] CLOSSING GRIPPER. Gripper Value: {}".format(gripper_value))				
					await asyncio.sleep(0.1)
				...
				send_over_data_channel("[WARNING] OPENING GRIPPER")				
				send_over_data_channel("[WARNING] Please wai for 3 seconds ...")								
...
if __name__ == "__main__":
    ...
    # ************* ROS **********
    rospy.init_node("arm_gripper_web_joystick_node")
    rospy.loginfo("Arm Node & Websocket client node initialized")
