...
#     joy_joint_values[1] += joy.axes[1] * increment # Joint 1 control
#     joy_joint_values[2] += joy.axes[3] * increment # Joint 1 control
#     joy_joint_values[3] += joy.axes[4] * increment # Joint 1 control
#     joy_joint_values[4] += joy.axes[6] * increment * 5.0 # Joint 1 control
#     joy_joint_values[5] += joy.axes[7] * increment * 5.0 # Joint 1 control
...
def joystick_control():
    ...
    try:
        ...
        # Joystick keys call different robot motions.
        while not rospy.is_shutdown():
            try:
                if key == "s":
                    set_coords(init_pose)
                    time.sleep(1)
                    set_coords(home_pose)
                    time.sleep(1)
                    set_coords(my_pose)
