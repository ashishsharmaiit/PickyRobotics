	...
	while not rospy.is_shutdown():
		...
		if gripper_command:
			if g == 0:
				...
				send_over_data_channel(json.dumps("SUCTION IS OFF ..."))
				send_over_data_channel(json.dumps("Please wait for 1 second ..."))																				
				await asyncio.sleep(1)
				g = 1
			elif g == 1:
				# Turn ON suction.
				...
				# print("Suction is ON ...")
				# rospy.logwarn("Please wait for 3 seconds!")								
				send_over_data_channel(json.dumps("SUCTION IS ON ..."))																
				send_over_data_channel(json.dumps("Please wait for 1 second ..."))								
				await asyncio.sleep(1)
				g = 0				
			else:
